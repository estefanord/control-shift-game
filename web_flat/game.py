...
class MusicManager:
    def __init__(self):
        print("DEBUG: Initializing MusicManager")
        try:
            ...
            print("DEBUG: MusicManager initialized")
        except Exception as e:
            print(f"DEBUG ERROR: MusicManager init failed: {str(e)}")
            self.music_files = []
            self.current_index = -1

    def play(self, index):
        print(f"DEBUG: Attempting to play music index {index}")
        try:
            if index != self.current_index and 0 <= index < len(self.music_files):
                ...
                try:
                    if os.path.exists(self.music_files[index]):
                        ...
                        print(f"DEBUG: Playing music file {self.music_files[index]}")
                    else:
                        print(f"DEBUG: Music file not found: {self.music_files[index]}")
                        self.current_index = -1
                except Exception as e:
                    print(f"DEBUG ERROR: Failed to load/play music: {str(e)}")
                    self.current_index = -1
        except Exception as e:
            print(f"DEBUG ERROR: Music play failed: {str(e)}")
            self.current_index = -1
    ...
    def stop(self):
        ...
        try:
            ...
            print(f"DEBUG ERROR: Music stop failed: {str(e)}")
            self.current_index = -1
...
class Game:
    def __init__(self):
        print("DEBUG: Game.__init__ started")
        try:
            # Initialize display with web-specific flags
            ...
            pygame.display.set_caption("Control Chaos")
            print("DEBUG: Display setup complete")

            # Initialize fonts with error handling
            print("DEBUG: Initializing fonts")
            try:
                pygame.font.init()
                self.font = pygame.font.Font(None, 32)
                self.large_font = pygame.font.Font(None, 64)
                print("DEBUG: Fonts initialized")
            except Exception as e:
                print(f"DEBUG ERROR: Font initialization failed: {str(e)}")
                print("DEBUG: Falling back to system fonts")
                try:
                    self.font = pygame.font.SysFont('Arial', 32)
                    self.large_font = pygame.font.SysFont('Arial', 64)
                    print("DEBUG: System fonts initialized")
                except Exception as e:
                    print(f"DEBUG ERROR: System font fallback failed: {str(e)}")
                    raise

            # Initialize music with error handling
            print("DEBUG: Initializing music")
            try:
                pygame.mixer.init(frequency=44100, size=-16, channels=2, buffer=512)
                print("DEBUG: Music initialized")
            except Exception as e:
                print(f"DEBUG ERROR: Music initialization failed: {str(e)}")
                print("DEBUG: Continuing without music")
            ...
            # Create room manager
            print("DEBUG: Creating room manager")
            self.room_manager = RoomManager(self.player)
            print("DEBUG: Room manager created")

            # Create game objects
            print("DEBUG: Creating game objects")
            ...
            print(f"DEBUG ERROR: Game initialization failed: {str(e)}")
            print("DEBUG: Stack trace:")
            traceback.print_exc()
            raise

    def handle_events(self):
        print("DEBUG: Handling events")
        try:
            for event in pygame.event.get():
                # Handle web-specific events
                if is_web_platform():
                    if event.type == pygame.VIDEORESIZE:
                        print("DEBUG: Handling resize event")
                        width = max(event.w, SCREEN_WIDTH)
                        height = max(event.h, SCREEN_HEIGHT)
                        self.screen = pygame.display.set_mode((width, height), pygame.RESIZABLE)
                    elif event.type in (pygame.BLUR, pygame.FOCUS):
                        print(f"DEBUG: Handling focus event: {event.type}")
                        continue
                ...
                if self.title_screen and event.type == pygame.KEYDOWN:
                    print("DEBUG: Title screen key event")
                ...
                if self.level_select_screen and event.type == pygame.KEYDOWN:
                    print(f"DEBUG: Level select key event: {event.key}")
                    if event.key == pygame.K_SPACE:
                        ...
                        if self.music_manager:
                            self.music_manager.play(level_num)
                    return
                
                # Pass events to room manager when playing
                if self.game_state == "PLAYING":
                    self.room_manager.handle_event(event)
        except Exception as e:
            print(f"DEBUG ERROR: Event handling failed: {str(e)}")

    def draw_text(self, text, x, y, color=(255,255,255), size=32):
        try:
            ...
            print(f"DEBUG ERROR: Text rendering failed: {str(e)}")
        ...
        try:
            ...
            while running:
                ...
                try:
                    ...
                    print(f"DEBUG ERROR: Game loop error: {str(e)}")
                    running = False
                    break
            ...
            print("DEBUG: Game loop ended normally")
        except Exception as e:
            print(f"DEBUG ERROR: Game.run failed: {str(e)}")
            print("DEBUG: Stack trace:")
            traceback.print_exc()
            raise
        finally:
            print("DEBUG: Cleaning up pygame")
            pygame.quit()
